//! Standard BIP-39 English wordlist with compile-time verification and O(log N) lookup.

/// Complete, compile-time verified BIP-39 English wordlist (exactly 2048 words).
pub static BIP39_ENGLISH_WORDLIST: [&str; 2048] = [
...
// Compile-time static assertions ensuring exact 2048 words
const _: () = {
    assert!(
        BIP39_ENGLISH_WORDLIST.len() == 2048,
        "BIP-39 wordlist must have exactly 2048 words"
    );
};

/// Lookup word index in O(log N) binary search time.
#[inline]
pub fn find_word_index(word: &str) -> Option<usize> {
    BIP39_ENGLISH_WORDLIST.binary_search(&word).ok()
}

/// Lookup word by index in O(1) constant time.
#[inline]
pub fn word_by_index(index: usize) -> Option<&'static str> {
    BIP39_ENGLISH_WORDLIST.get(index).copied()
}
...
mod tests {
    ...
    #[test]
    fn wordlist_has_exact_2048_words() {
        assert_eq!(BIP39_ENGLISH_WORDLIST.len(), 2048);
        assert_eq!(BIP39_ENGLISH_WORDLIST[0], "abandon");
        assert_eq!(BIP39_ENGLISH_WORDLIST[2047], "zoo");
    }

    #[test]
    fn wordlist_is_strictly_sorted() {
        for i in 1..BIP39_ENGLISH_WORDLIST.len() {
            assert!(
                BIP39_ENGLISH_WORDLIST[i - 1] < BIP39_ENGLISH_WORDLIST[i],
                "Wordlist ordering invariant broken at index {}: {} >= {}",
                i,
                BIP39_ENGLISH_WORDLIST[i - 1],
                BIP39_ENGLISH_WORDLIST[i]
            );
        }
    }

    #[test]
    fn binary_search_lookup_works() {
        assert_eq!(find_word_index("abandon"), Some(0));
        assert_eq!(find_word_index("zoo"), Some(2047));
        // "satoshi" is a canonical BIP-39 word (index 1531).
        assert_eq!(find_word_index("satoshi"), Some(1531));
        assert_eq!(find_word_index("nonexistentword"), None);
        assert_eq!(find_word_index(""), None);
        assert_eq!(find_word_index("ABANDON"), None); // lookup is lowercase-only
    }

    #[test]
    fn wordlist_matches_canonical_checksum() {
        // SHA-256 of the canonical TREZOR BIP-39 English wordlist joined by
        // single spaces. Guards against silent corruption: a single missing or
        // substituted word shifts mnemonic<->entropy mappings for every index
        // after it while remaining alphabetically sorted, so the sort test
        // alone cannot detect it.
        let joined = BIP39_ENGLISH_WORDLIST.join(" ");
        let digest = sha2::Sha256::digest(joined.as_bytes());
        assert_eq!(
            hex::encode(digest),
            "f18b9a84c83e38e98eceb0102b275e26438af83ab08f080cdb780a2caa9f3a6d"
        );
    }

    #[test]
    fn official_vector_anchor_indices() {
        // Indices exercised by the official BIP-39 test vectors (7f*16 / 80*16),
        // cross-checked against the canonical TREZOR list.
        assert_eq!(find_word_index("legal"), Some(1019));
        assert_eq!(find_word_index("winner"), Some(2015));
        assert_eq!(find_word_index("thank"), Some(1790));
        assert_eq!(find_word_index("letter"), Some(1028));
        assert_eq!(find_word_index("advice"), Some(32));
        assert_eq!(find_word_index("cage"), Some(257));
    }
}
